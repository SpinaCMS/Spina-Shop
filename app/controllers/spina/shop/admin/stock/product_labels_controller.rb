      ...
      class ProductLabelsController < AdminController
        ...
        def new
          ...
          @product_labels = @products.map do |product|
            {tht: (l(product.expiration_date, format: "%m-%Y") if product.expiration_date), name: product.full_name, location: product.location, product: product, locations: [product.product_locations.pluck(:location_code), product.location].map(&:presence).compact.uniq}
          end
        end
