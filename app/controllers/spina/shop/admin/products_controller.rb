module Spina::Shop
  module Admin
    class ProductsController < AdminController
      ...
      def index
        @q = Product.where(archived: false).filtered(filters).order(created_at: :desc).includes(:stores, :product_images).joins(:translations).where(spina_shop_product_translations: {locale: I18n.locale}).ransack(params[:q])
